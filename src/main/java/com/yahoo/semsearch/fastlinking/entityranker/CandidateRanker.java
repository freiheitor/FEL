...
public abstract class CandidateRanker {
     ...
     */
    public ArrayList<EntityScore> getTopKEntities( CandidatesInfo infos, EntityContext context, String surfaceForm, int length, int k, String mention, int start, int end ) {
        ...
        for ( int i = 0; i < k; i++ ) {
            scoresTop.add( new EntityScore( infos.entities[ idx[ i ] ], scores[ idx[ i ] ], mention, start, end ) );
        }
        return scoresTop;
    }
